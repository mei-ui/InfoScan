...
if (is.null(opt$finall_fpkm)||is.null(opt$min_gene) || is.null(opt$all_id)|| is.null(opt$max_gene)|| is.null(opt$before_qc)|| 
		...
		is.null(opt$tsne)|| is.null(opt$umap)|| is.null(opt$scRNA_rds)||is.null(opt$pct_mt)||is.null(opt$meta_data)||is.null(opt$resolution_chose)){
  print_help(opt_parser);
  stop("Please provide -a finall fpkm, -b all_id ,-c before_qc,-d after_qc,-e VariableFeatures ,-f pca,-g cell_cluster and -l tSNE,
 				-i UMAP,-j meta_data ,-k pearplot,-m scRNA_rds,-n min_gene,-p max_gene,-q pct_mt,-t pca_num,-r resolution_chose", call.=FALSE);
}
...
all_id<-read.table(ALLid,header=FALSE)
...
meatdata<-inner_join(metadata,dat_1,by=c("sample"="id"))
...
mt=PercentageFeatureSet(scRNA, pattern = "^mt-")
if(apply(mt,2,sum)==0){scRNA[["percent.mt"]] <- PercentageFeatureSet(scRNA, pattern = "^MT-")}else if(apply(mt,2,sum)!=0){scRNA[["percent.mt"]] <- PercentageFeatureSet(scRNA, pattern = "^mt-")}
...
##10降维之TSNE
set.seed(888)
scRNA = RunTSNE(scRNA, dims = pc.num,perplexity=10)
...
##UMAP
scRNA <- RunUMAP(scRNA, dims = pc.num,perplexity=10)

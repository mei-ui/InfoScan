...
option_list <- list(
                ...
                make_option(c("-m","--lncRNA_heatmap"),type="character", default=NULL, help="output top10 lprotein marker gene file"),
...
protein_coding<-scRNA[which(rownames(scRNA)%in%c(protein_id)),]


protein_coding <- NormalizeData(protein_coding, normalization.method = "LogNormalize", scale.factor = 10000)
protein_coding <- FindVariableFeatures(protein_coding, selection.method = "vst", nfeatures = 2000)
top10 <- head(VariableFeatures(protein_coding), 10)
plot1 <- VariableFeaturePlot(protein_coding)
plot2 <- LabelPoints(plot = plot1, points = top10, repel = TRUE, size=2.5)
plot <- CombinePlots(plots = list(plot1, plot2),legend="bottom")
ggsave(VariableFeatures, plot = plot, width = 8, height = 6)

##07标准化
scale.genes <-  rownames(protein_coding)
protein_coding<- ScaleData(protein_coding, features = scale.genes)
# 结果存储在scRNA[["RNA"]]@scale.data中

##08PCA降维
protein_coding<- RunPCA(protein_coding,features=VariableFeatures(protein_coding))
plot1 <- DimPlot(protein_coding, reduction = "pca")
plot2 <- ElbowPlot(protein_coding, ndims=20, reduction="pca")
plotc <- plot1+plot2
...
Resolution
protein_coding <- FindNeighbors(protein_coding, dims = pc.num)
protein_coding <- FindClusters(protein_coding, resolution=Resolution)
table(protein_coding@meta.data$seurat_clusters)
metadata <- protein_coding@meta.data
cell_cluster <- data.frame(cell_ID=rownames(metadata), cluster_ID=metadata$seurat_clusters)
write.csv(cell_cluster,CELLcluster,row.names = F)

##10降维之TSNE
set.seed(888)
protein_coding = RunTSNE(protein_coding, dims = pc.num,perplexity=10)
embed_tsne <- Embeddings(protein_coding, 'tsne')
plot1 = DimPlot(protein_coding, reduction = "tsne",label = T)
ggsave(TSNE, plot = plot1, width = 8, height = 7)
p1 = DimPlot(protein_coding, reduction = "tsne",label = F,group.by=group)
ggsave(TSNE_tissue, plot = p1, width = 10, height = 7)


##UMAP
protein_coding <- RunUMAP(protein_coding, dims = pc.num,perplexity=10)
embed_umap <- Embeddings(protein_coding, 'umap')
plot2 = DimPlot(protein_coding, reduction = "umap",label = T)
ggsave(UMAP, plot = plot2, width = 8, height = 7)
p2 = DimPlot(protein_coding, reduction = "umap",label = F,group.by=group)
ggsave(UMAP_tissue, plot = p2, width = 10, height = 7)

diff.wilcox = FindAllMarkers(protein_coding)
all.markers = diff.wilcox %>% select(gene, everything()) %>% subset(p_val<0.05)
top10 = all.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)
top10_genes = CaseMatch(search = as.vector(top10$gene), match = rownames(protein_coding ))
plot1 = DoHeatmap(protein_coding, features = top10_genes, group.by = "seurat_clusters", group.bar = T, size = 4)
ggsave(LncRNAheatmap, plot=plot1, width=15, height=18)
out_dir=paste0(OutDir,"/snakemake/result/11_data_analysis/cluster/protein/protein.rds")
saveRDS(protein_coding,file=out_dir)
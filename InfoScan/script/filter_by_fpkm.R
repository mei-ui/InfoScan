...
if (is.null(opt$annotated_count)|| is.null(opt$unannotated_count) || is.null(opt$fpkm_matrix)||is.null(opt$all_id) 
  ...
  stop("Please provide -a finall_count, -b all_id ,-c protein_coding,-d annotated_lncRNA ,-e unannotated_lncRNA ,-f unannotated_protein,
        -g length_density and -i length_boxplot", call.=FALSE);
}
